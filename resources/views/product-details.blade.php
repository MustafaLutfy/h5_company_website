            ...
            <div class="lg:col-span-2">
              ...
              <div class="flex flex-wrap gap-4 mt-6">
                @if($product->original_price == $product->new_price)
                  <p class="text-[#333] text-4xl font-bold">{{$product->original_price}} IQD</p>
                
                @else
                  <p class="text-[#333] text-4xl font-bold">{{$product->new_price}} IQD</p>
                  <p class="text-gray-400 text-xl"><strike>{{$product->original_price}} IQD</strike></p>  
              ...
              <button type="button" class="min-w-[200px] px-4 py-2.5 border border-[#333] bg-transparent hover:bg-gray-50 text-[#333] text-sm font-bold rounded-md">Add to cart</button>
              </div>
            </div>
          </div>
    ...
    <form action="{{route('order' , $id)}}" method="POST">
      @csrf
        <div class="space-y-4">
            ...
            <div>
                <label for="government" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Government</label>
